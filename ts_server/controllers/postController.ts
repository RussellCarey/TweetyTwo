...
exports.uploadTweet = catchAsync(async (req: IReqWithBody, res: Response, next: NextFunction) => {
	...
	const userDataObject = await DatabaseServies.getUserData(req.user.id);
	...
	Schedule.createNewJob(twitter_id, email, display_name, message, unixNumber, imageURL, imageName, access, refresh, true, null);
