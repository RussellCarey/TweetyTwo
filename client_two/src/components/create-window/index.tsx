...
const CreateWindow: FunctionComponent = () => {
	...
	const dataChangeHandler = (e: React.ChangeEvent) => {
		...
		const unix = convertTimeDateToUnix(target.id === 'date' ? target.value : tweet.date, target.id === 'time' ? target.value : tweet.time);
	...
	const handleSubmitClick = async () => {
		...
		if (uploadImageAttempt === ERequestOutcomes.hasError) return showModal('Error creating tweety. Please try again.', EModal.hasError);
		...
		if (submittedTweet === ERequestOutcomes.hasLimitError) return showModal('Reached upload limit. Please try again in one hour.', EModal.hasError);
	...
	return (
		<Container style={animationProps}>
			...
			<InputsContainer>
				...
				<FileInput id="file-select" type="file" accept="image/*" ref={imageInputElement} onChange={imageOnChangeHandler} />
			</InputsContainer>
		</Container>
	);
};
